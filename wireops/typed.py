...
T_PUT_FUNCS[FieldTypes.V_ENUM.value[0]] = venum_put


def vuint32_put(chan, val, nnn):
    varint_ = 0xffffffff & val
    write_varint_field(chan, varint_, nnn)
T_PUT_FUNCS[FieldTypes.V_UINT32.value[0]] = vuint32_put


def vsint32_put(chan, val, nnn):
    varint_ = encode_sint32(val)
    write_varint_field(chan, varint_, nnn)
T_PUT_FUNCS[FieldTypes.V_SINT32.value[0]] = vsint32_put


def vuint64_put(chan, val, nnn):
    varint_ = 0xffffffffffffffff & val
    write_varint_field(chan, varint_, nnn)
T_PUT_FUNCS[FieldTypes.V_UINT64.value[0]] = vuint64_put


def vsint64_put(chan, val, nnn):
    varint_ = encode_sint64(val)
    write_varint_field(chan, varint_, nnn)
T_PUT_FUNCS[FieldTypes.V_SINT64.value[0]] = vsint64_put
...
T_PUT_FUNCS[FieldTypes.F_UINT32.value[0]] = fuint32_put


def fsint32_put(chan, val, nnn):
    val = ctypes.c_int32(val).value
    write_b32_field(chan, val, nnn)
T_PUT_FUNCS[FieldTypes.F_SINT32.value[0]] = fsint32_put
...
T_PUT_FUNCS[FieldTypes.F_FLOAT.value[0]] = ffloat_put
...
T_PUT_FUNCS[FieldTypes.F_UINT64.value[0]] = fuint64_put


def fsint64_put(chan, val, nnn):
    varint_ = ctypes.c_int64(val).value
    write_b64_field(chan, varint_, nnn)
T_PUT_FUNCS[FieldTypes.F_SINT64.value[0]] = fsint64_put
...
T_PUT_FUNCS[FieldTypes.L_STRING.value[0]] = l_string_put


def lbytes_put(chan, val, nnn):
    return write_len_plus_field(chan, val, nnn)
T_PUT_FUNCS[FieldTypes.L_BYTES.value[0]] = lbytes_put
...
def lmsg_put(chan, val, nnn):
    raise NotImplementedError
T_PUT_FUNCS[FieldTypes.L_MSG.value[0]] = lmsg_put


def fbytes16_put(chan, val, nnn):
    return write_b128_field(chan, val, nnn)
T_PUT_FUNCS[FieldTypes.F_BYTES16.value[0]] = fbytes16_put


def fbytes20_put(chan, val, nnn):
    return write_b160_field(chan, val, nnn)
T_PUT_FUNCS[FieldTypes.F_BYTES20.value[0]] = fbytes20_put
...
T_GET_FUNCS[FieldTypes.V_ENUM.value[0]] = venum_get
...
def vbool_get(chan):
    ...
    if varint_:
        ...
        return False
T_GET_FUNCS[FieldTypes.V_BOOL.value[0]] = vbool_get


def vuint32_get(chan):
    return read_raw_varint(chan)
T_GET_FUNCS[FieldTypes.V_UINT32.value[0]] = vuint32_get


def vsint32_get(chan):
    varint_ = read_raw_varint(chan)
    return decode_sint32(varint_)
T_GET_FUNCS[FieldTypes.V_SINT32.value[0]] = vsint32_get


def vuint64_get(chan):
    return read_raw_varint(chan)
T_GET_FUNCS[FieldTypes.V_UINT64.value[0]] = vuint64_get


def vsint64_get(chan):
    varint_ = read_raw_varint(chan)
    return decode_sint64(varint_)
T_GET_FUNCS[FieldTypes.V_SINT64.value[0]] = vsint64_get              # END VAR
...
T_GET_FUNCS[FieldTypes.F_UINT32.value[0]] = fuint32_get


def fsint32_get(chan):
    return read_raw_b32(chan)
T_GET_FUNCS[FieldTypes.F_SINT32.value[0]] = fuint32_get
...
def ffloat_get(chan):
    ...
    return val
T_GET_FUNCS[FieldTypes.F_FLOAT.value[0]] = ffloat_get
...
T_GET_FUNCS[FieldTypes.F_UINT64.value[0]] = fuint64_get


def fsint64_get(chan):
    return read_raw_b64(chan)
T_GET_FUNCS[FieldTypes.F_SINT64.value[0]] = fuint64_get
...
def fdouble_get(chan):
    ...
    return val
T_GET_FUNCS[FieldTypes.F_DOUBLE.value[0]] = fdouble_get
...
def lstring_get(chan):
    ...
    # END
    return value
T_GET_FUNCS[FieldTypes.L_STRING.value[0]] = lstring_get


def lbytes_get(chan):
    return read_raw_len_plus(chan)
T_GET_FUNCS[FieldTypes.L_BYTES.value[0]] = lbytes_get


def lmsg_get(chan):
    # caller must interpret the raw byte array
    return read_raw_len_plus(chan)
T_GET_FUNCS[FieldTypes.L_MSG.value[0]] = lmsg_get
...
T_GET_FUNCS[FieldTypes.F_BYTES16.value[0]] = fbytes16_get


def fbytes20_get(chan):
    return read_raw_b160(chan)
T_GET_FUNCS[FieldTypes.F_BYTES20.value[0]] = fbytes20_get


def fbytes32_get(chan):
    return read_raw_b256(chan)
T_GET_FUNCS[FieldTypes.F_BYTES32.value[0]] = fbytes32_get
...
T_LEN_FUNCS[FieldTypes.V_BOOL.value[0]] = vbool_len
...
T_LEN_FUNCS[FieldTypes.V_ENUM.value[0]] = venum_len
...
T_LEN_FUNCS[FieldTypes.L_STRING.value[0]] = l_string_len
...
def lbytes_len(val, ndx):
    ...
    v_len = len(val)
    return h_len + length_as_varint(v_len) + v_len
T_LEN_FUNCS[FieldTypes.L_BYTES.value[0]] = lbytes_len
...
def lmsg_len(val, ndx):
    ...
    v_len = val.wire_len
    return h_len + length_as_varint(v_len) + v_len
T_LEN_FUNCS[FieldTypes.L_MSG.value[0]] = lmsg_len
...
def fbytes16_len(val, ndx):
    ...
    return h_len + 16
T_LEN_FUNCS[FieldTypes.F_BYTES16.value[0]] = fbytes16_len
...
def fbytes20_len(val, ndx):
    ...
    return h_len + 20
T_LEN_FUNCS[FieldTypes.F_BYTES20.value[0]] = fbytes20_len
...
def fbytes32_len(val, ndx):
    ...
    return h_len + 32
T_LEN_FUNCS[FieldTypes.F_BYTES32.value[0]] = fbytes32_len
